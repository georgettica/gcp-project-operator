...
func init() {
	...
	utilruntime.Must(gcpmanagedopenshiftiov1alpha1.AddToScheme(scheme))
...
func main() {
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:   "f463560a.my.domain",
	})
	if err != nil {
	...
	if err = (&controllers.ProjectReferenceReconciler{
		...
		Log:    ctrl.Log.WithName("controllers").WithName("ProjectReference"),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "ProjectReference")
		os.Exit(1)
	}
	if err = (&controllers.ProjectClaimReconciler{
		Client: mgr.GetClient(),
		Log:    ctrl.Log.WithName("controllers").WithName("ProjectClaim"),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
