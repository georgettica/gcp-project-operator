...
// Regions supported in the gcrg p-project-operator
...
func (c *ProjectClaimAdapter) EnsureFinalizerDeleted() error {
	...
	if gcputil.Contains(finalizers, ProjectClaimFinalizer) {
		c.projectClaim.SetFinalizers(gcputil.Filter(finalizers, ProjectClaimFinalizer))
